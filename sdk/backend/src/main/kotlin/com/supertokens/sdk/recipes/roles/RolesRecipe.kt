...
class RolesRecipe(private val superTokens: SuperTokens, private val config: RolesRecipeConfig) :
  ...
  override suspend fun getExtraJwtData(
    ...
    val userPermissions =
        if (addPermissionsToToken) {
          buildSet { userRoles.forEach { role -> addAll(getRolePermissions(role)) } }
        } else emptySet()
  ...
  suspend fun createOrUpdateRole(role: String, permissions: List<String>): Boolean {
    val response =
        superTokens.put(PATH_ROLE, tenantId = null) {
    ...
    return response.parse<CreateOrUpdateRoleResponseDTO, Boolean> { it.createdNewRole }
  }
  ...
  suspend fun deleteRole(role: String): Boolean {
    val response =
        superTokens.post(PATH_ROLES_REMOVE, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_ROLES)
    ...
    return response.parse<DeleteRoleResponseDTO, Boolean> { it.didRoleExist }
  }
  ...
  suspend fun getRoles(): List<String> {
    val response =
        superTokens.get(PATH_ROLES, tenantId = null) { header(HEADER_RECIPE_ID, RECIPE_ROLES) }

    return response.parse<GetRolesResponseDTO, List<String>> { it.roles }
  }
  ...
  suspend fun getRolePermissions(role: String): List<String> {
    ...
    return response.parse<GetRolePermissionsResponseDTO, List<String>> { it.permissions }
  }
  ...
  suspend fun getRoleUsers(role: String, tenantId: String?): List<String> {
    ...
    return response.parse<GetRoleUsersResponseDTO, List<String>> { it.users }
  }

  /**
   * Removes permissions mapped to a role, if no permissions are passed all permissions mapped to
   * the role are removed
   */
  @Throws(SuperTokensStatusException::class)
  suspend fun removeRolePermissions(role: String, permissions: List<String>): SuperTokensStatus {
    val response =
        superTokens.post(PATH_ROLES_PERMISSIONS_REMOVE, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_ROLES)
  ...
  suspend fun setUserRole(userId: String, role: String, tenantId: String?): Boolean {
    val response =
        superTokens.put(PATH_USER_ROLE, tenantId = tenantId) {
          header(HEADER_RECIPE_ID, RECIPE_ROLES)
    ...
    return response.parse<SetUserRoleResponseDTO, Boolean> { it.didUserAlreadyHaveRole == true }
  }
  ...
  suspend fun removeUserRole(userId: String, role: String, tenantId: String?): Boolean {
    val response =
        superTokens.post(PATH_USER_ROLE_REMOVE, tenantId = tenantId) {
          header(HEADER_RECIPE_ID, RECIPE_ROLES)
    ...
    return response.parse<RemoveUserRoleResponseDTO, Boolean> { it.didUserHaveRole == true }
  }
...
val Roles =
    object : RecipeBuilder<RolesRecipeConfig, RolesRecipe>() {
...
/** Retrive all created roles */
suspend fun SuperTokens.getRoles() = getRecipe<RolesRecipe>().getRoles()
...
/**
 * Removes permissions mapped to a role, if no permissions are passed all permissions mapped to the
 * role are removed
 */
