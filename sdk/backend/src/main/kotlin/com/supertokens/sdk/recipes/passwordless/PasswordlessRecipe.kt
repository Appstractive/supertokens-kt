...
class PasswordlessRecipe(
  ...
  /**
   * Starts a sign in process by requesting a linkCode and a deviceId + userInputCode combination
   * the user can use to sign in.
   */
  @Throws(SuperTokensStatusException::class)
  suspend fun createEmailCode(email: String, tenantId: String?): PasswordlessCodeData {
    val response =
        superTokens.post(PATH_CREATE_CODE, tenantId = tenantId) {
   ...
   */
  @Throws(SuperTokensStatusException::class)
  suspend fun createDeviceIdCode(
      deviceId: String,
  ...
  suspend fun consumeLinkCode(
    ...
    val response =
        superTokens.post(PATH_CONSUME_CODE, tenantId = tenantId) {
          header(HEADER_RECIPE_ID, RECIPE_PASSWORDLESS)
    ...
    return response
        .parse<ConsumePasswordlessCodeResponseDTO, SignInUpData> {
          SignInUpData(
  ...
  @Throws(SuperTokensStatusException::class)
  suspend fun consumeUserInputCode(
      preAuthSessionId: String,
      deviceId: String,
      userInputCode: String,
      tenantId: String?
    ...
    return response
        .parse<ConsumePasswordlessCodeResponseDTO, SignInUpData> {
          SignInUpData(
              ...
              createdNewUser =
                  it.createdNewUser
                      ?: throw RuntimeException("OK StatusResponse without createdNewUser"),
          )
        }
        .also {
          if (it.createdNewUser) {
  ...
  suspend fun revokeCode(codeId: String, tenantId: String?): SuperTokensStatus {
    val response =
        superTokens.post(PATH_REVOKE_CODE, tenantId = tenantId) {
          header(HEADER_RECIPE_ID, RECIPE_PASSWORDLESS)
  ...
  suspend fun revokeEmailCodes(email: String, tenantId: String?): SuperTokensStatus {
    val response =
        superTokens.post(PATH_REVOKE_CODES, tenantId = tenantId) {
          header(HEADER_RECIPE_ID, RECIPE_PASSWORDLESS)
  ...
  suspend fun getCodesByEmail(email: String, tenantId: String?): List<PasswordlessDevices> {
    val response =
        superTokens.get(
            PATH_GET_CODES,
            tenantId = tenantId,
            queryParams = mapOf("email" to email),
        ) {
          header(HEADER_RECIPE_ID, RECIPE_PASSWORDLESS)
        }

    return response.parse<GetPasswordlessCodesResponseDTO, List<PasswordlessDevices>> { it.devices }
  }
  ...
  suspend fun getCodesByDeviceId(deviceId: String, tenantId: String?): List<PasswordlessDevices> {
    val response =
        superTokens.get(
            PATH_GET_CODES,
            tenantId = tenantId,
            queryParams = mapOf("deviceId" to deviceId),
        ) {
          header(HEADER_RECIPE_ID, RECIPE_PASSWORDLESS)
        }
  ...
  /** Lists all active passwordless codes of the user */
  @Throws(SuperTokensStatusException::class)
  suspend fun getCodesByPhoneNumber(
      phoneNumber: String,
      tenantId: String?
  ): List<PasswordlessDevices> {
    val response =
        superTokens.get(
            PATH_GET_CODES,
            tenantId = tenantId,
            queryParams = mapOf("phoneNumber" to phoneNumber),
        ) {
          header(HEADER_RECIPE_ID, RECIPE_PASSWORDLESS)
        }
  ...
  /** Lists all active passwordless codes of the user */
  @Throws(SuperTokensStatusException::class)
  suspend fun getCodesByPreAuthSessionId(
      preAuthSessionId: String,
      tenantId: String?
  ): List<PasswordlessDevices> {
    val response =
        superTokens.get(
            PATH_GET_CODES,
            tenantId = tenantId,
            queryParams = mapOf("preAuthSessionId" to preAuthSessionId),
        ) {
          header(HEADER_RECIPE_ID, RECIPE_PASSWORDLESS)
        }
  ...
  suspend fun updatePhoneNumber(
    ...
    return response.parse().also {
      superTokens._events.tryEmit(
          SuperTokensEvent.UserPhoneNumberChanged(userId = userId, phoneNumber = phoneNumber))
    }
  }
...
val Passwordless =
    object : RecipeBuilder<PasswordlessRecipeConfig, PasswordlessRecipe>() {

      override fun install(
          configure: PasswordlessRecipeConfig.() -> Unit
      ): (SuperTokens) -> PasswordlessRecipe {
...
/**
 * Starts a sign in process by requesting a linkCode and a deviceId + userInputCode combination the
 * user can use to sign in.
 */
suspend fun SuperTokens.createPasswordlessEmailCode(
    email: String,
    tenantId: String? = null,
) =
    getRecipe<PasswordlessRecipe>()
        .createEmailCode(
            email = email,
            tenantId = tenantId,
        )
