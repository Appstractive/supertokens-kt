...
class BitbucketProvider(
  ...
  override val id = ThirdPartyAuth.BITBUCKET
  ...
  override suspend fun getUserInfo(tokenResponse: ThirdPartyTokensDTO): ThirdPartyUserInfo {
    val response = superTokens.client.get(USER_URL) { bearerAuth(tokenResponse.accessToken) }
    ...
    return ThirdPartyUserInfo(
        ...
        email =
            getEmail(tokenResponse.accessToken)?.let {
              ThirdPartyEmail(id = it.email, isVerified = it.is_confirmed)
            })
  }

  private suspend fun getEmail(accessToken: String): BitbucketGetEmailsResponse? {
    val response = superTokens.client.get(EMAIL_URL) { bearerAuth(accessToken) }
...
val Bitbucket =
    object : ProviderBuilder<BitbucketConfig, BitbucketProvider>() {

      override fun install(
          configure: BitbucketConfig.() -> Unit
      ): (SuperTokens, ThirdPartyRecipe) -> BitbucketProvider {
