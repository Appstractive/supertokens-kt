...
class SessionRecipe(private val superTokens: SuperTokens, private val config: SessionConfig) :
    Recipe<SessionConfig> {

  // if true, attach tokens to response headers
  val headerBasedSessions by lazy { config.headerBasedSessions }

  // if true, set tokens to cookies
  val cookieBasedSessions by lazy { config.cookieBasedSessions }

  val cookieDomain by lazy { config.cookieDomain ?: superTokens.appConfig.api.host }

  val cookiePath by lazy { config.cookiePath ?: superTokens.appConfig.api.path }

  val secureCookies by lazy { superTokens.appConfig.api.scheme == "https" }

  val cookieSameSite by lazy {
    // will be 'none' if https is used and frontend and api are different hosts, else 'lax'
    if (secureCookies && cookieDomain != superTokens.appConfig.api.host) {
      "none"
    } else {
      "lax"
    }
  ...
  // the JWT issuer to use
  val issuer by lazy { config.issuer ?: superTokens.appConfig.api.host }
  ...
  suspend fun getJwtData(
    ...
    user.email?.let { set(Claims.EMAIL, it) }
    user.phoneNumber?.let { set(Claims.PHONE_NUMBER, it) }
    ...
    customJwtData?.let {
      it.invoke(superTokens, user).forEach { entry -> set(entry.key, entry.value) }
    }
  }
  ...
  suspend fun createSession(
    ...
    val response =
        superTokens.post(PATH_SESSION, tenantId = tenantId) {
  ...
  suspend fun getSession(sessionHandle: String): GetSessionData {
    ...
    return response.parse<GetSessionResponseDTO, GetSessionData> {
      GetSessionData(
          ...
          userDataInDatabase =
              it.userDataInDatabase?.entries?.associate { entry ->
                entry.key to entry.value.extractedContent
              },
          userDataInJWT =
              it.userDataInJWT?.entries?.associate { entry ->
                entry.key to entry.value.extractedContent
              },
      )
    }
  ...
  suspend fun getSessions(userId: String, tenantId: String?): List<String> {
    ...
    return response.parse<GetSessionsResponseDTO, List<String>> { it.sessionHandles }
  }
  ...
  suspend fun removeSessions(sessionHandles: List<String>, tenantId: String?): List<String> {
    ...
    val response =
        superTokens.post(PATH_SESSION_REMOVE, tenantId = tenantId) {
          header(HEADER_RECIPE_ID, RECIPE_SESSION)
    ...
    return response.parse<RemoveSessionsResponseDTO, List<String>> { it.sessionHandlesRevoked }
  }
  ...
  suspend fun verifySession(
    ...
    val response =
        superTokens.post(PATH_SESSION_VERIFY, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_SESSION)
  ...
  suspend fun refreshSession(
    ...
    val response =
        superTokens.post(PATH_SESSION_REFRESH, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_SESSION)
  ...
  suspend fun regenerateSession(
    ...
    val response =
        superTokens.post(PATH_SESSION_REGENERATE, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_SESSION)
  ...
  suspend fun updateSessionData(
    ...
    val response =
        superTokens.put(PATH_SESSION_DATA, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_SESSION)
  ...
  suspend fun updateJwtData(
    ...
    val response =
        superTokens.put(PATH_JWT_DATA, tenantId = tenantId) {
          header(HEADER_RECIPE_ID, RECIPE_SESSION)
...
val Sessions =
    object : RecipeBuilder<SessionConfig, SessionRecipe>() {
...
suspend fun SuperTokens.refreshSession(
    ...
    getRecipe<SessionRecipe>()
        .refreshSession(refreshToken = refreshToken, antiCsrfToken = antiCsrfToken)
...
suspend fun SuperTokens.regenerateSession(
    ...
    getRecipe<SessionRecipe>()
        .regenerateSession(accessToken = accessToken, userDataInJWT = userDataInJWT)
...
suspend fun SuperTokens.updateSessionData(
    ...
    getRecipe<SessionRecipe>()
        .updateSessionData(sessionHandle = sessionHandle, userDataInDatabase = userDataInDatabase)
