...
class UserMetaDataRecipe(
  ...
  /**
   * Updates the metadata object stored about the user by doing a shallow merge of the stored and
   * the update JSONs and removing properties set to null on the root level of the update object.
   * The merged object is then reserialized and stored. e.g.: stored: { "preferences": {
   * "theme":"dark" }, "notifications": { "email": true }, "todos": ["example"] } update: {
   * "notifications": { "sms": true }, "todos": null } result: { "preferences": { "theme":"dark" },
   * "notifications": { "sms": true } }
   */
  @Throws(SuperTokensStatusException::class)
  suspend fun updateMetaData(userId: String, metaData: Map<String, Any?>): Map<String, Any?> {
    val response =
        superTokens.put(PATH_META_DATA, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_META_DATA)
  ...
  suspend fun deleteMetaData(userId: String): SuperTokensStatus {
    val response =
        superTokens.post(PATH_META_DATA_REMOVE, tenantId = null) {
          header(HEADER_RECIPE_ID, RECIPE_META_DATA)
...
val UserMetaData =
    object : RecipeBuilder<UserMetaDataRecipeConfig, UserMetaDataRecipe>() {

      override fun install(
          configure: UserMetaDataRecipeConfig.() -> Unit
      ): (SuperTokens) -> UserMetaDataRecipe {
        val config = UserMetaDataRecipeConfig().apply(configure)

        return { UserMetaDataRecipe(it, config) }
      }
    }
...
/**
 * Updates the metadata object stored about the user by doing a shallow merge of the stored and the
 * update JSONs and removing properties set to null on the root level of the update object. The
 * merged object is then reserialized and stored. e.g.: stored: { "preferences": { "theme":"dark" },
 * "notifications": { "email": true }, "todos": ["example"] } update: { "notifications": { "sms":
 * true }, "todos": null } result: { "preferences": { "theme":"dark" }, "notifications": { "sms":
 * true } }
 */
