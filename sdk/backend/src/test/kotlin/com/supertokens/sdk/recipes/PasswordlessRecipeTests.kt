...
class PasswordlessRecipeTests : BaseTest() {
  ...
  fun testConsumeUserInputCode() = runBlocking {
    ...
    val response =
        superTokens.consumePasswordlessUserInputCode(
            code.preAuthSessionId, code.deviceId, code.userInputCode)
    assertEquals(TEST_USER, response.user.email)
  }
  ...
  fun testConsumeCodeFailed() = runBlocking {
    ...
    val exception =
        assertThrows(SuperTokensStatusException::class.java) {
          runBlocking {
            superTokens.consumePasswordlessUserInputCode(
                code.preAuthSessionId, code.deviceId, "wrong code")
          }
        }
  ...
  fun testGetCodesByEmail() = runBlocking {
    ...
    assertNotNull(codes.first().codes.firstOrNull { it.codeId == code.codeId })
