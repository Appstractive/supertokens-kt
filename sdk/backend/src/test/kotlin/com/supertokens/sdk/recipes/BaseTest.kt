...
abstract class BaseTest(connectionUrl: String = "http://localhost:3567/") {

  internal val superTokens =
      superTokens(
  ...
  fun setup(): Unit = runBlocking {
    val result = runCatching { superTokens.emailPasswordSignUp(TEST_USER, TEST_PASSWORD) }

    result.onFailure {
      if (it is SuperTokensStatusException &&
          it.status == SuperTokensStatus.EMailAlreadyExistsError) {
  ...
  suspend fun getRecipeUser(email: String, recipeId: String = RECIPE_EMAIL_PASSWORD): User {
    return superTokens.getUsersByEMail(email).first { user ->
      user.loginMethods?.any { method -> method.recipeId == recipeId } == true
    }
  }
