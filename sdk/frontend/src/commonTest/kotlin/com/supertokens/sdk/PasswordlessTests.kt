...
class PasswordlessTests {
  ...
  fun testSignUp() = runBlocking {
    val data = client.signUpWith(Passwordless) { email = "test@test.de" }
  ...
  fun testSignUpError() = runBlocking {
    val response = runCatching { client.signUpWith(Passwordless) {} }
  ...
  fun testSignInLinkCodeError() = runBlocking {
    val response = runCatching { client.signInWith(PasswordlessLinkCode) {} }
  ...
  fun testSignInInputCodeError() = runBlocking {
    val response = runCatching { client.signInWith(PasswordlessInputCode) {} }
