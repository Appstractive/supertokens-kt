...
open class MultiFactorHandler(
  ...
  open suspend fun RoutingContext.checkMfaStatus() {
    ...
    val hasPasswordlessPhone = hasPasswordless && passwordLessMethods.any { it.phoneNumber != null }
    ...
    val allowedSetup =
        if (sessionData.userDataInJWT.isMultiFactorAuthenticated || alreadySetup.isEmpty()) {
          getAllowedToSetup(
              hasTotpDevice = hasTotpDevice,
              hasPasswordlessEmail = hasPasswordlessEmail,
              hasPasswordlessPhone = hasPasswordlessPhone,
    ...
    // continue next with
    val missing =
        requiredSecondFactors.missing(factors).filter {
          // any already setup
