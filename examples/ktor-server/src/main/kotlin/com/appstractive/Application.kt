...
fun Application.module() {
  ...
  val superTokensUrl =
      environment.config.propertyOrNull("supertokens.url")?.getString()
          ?: throw IllegalStateException("supertokens.url not configured")
  val superTokensIssuer =
      environment.config.propertyOrNull("supertokens.issuer")?.getString() ?: "localhost"
  val superTokensDomain =
      environment.config.propertyOrNull("supertokens.domain")?.getString() ?: "localhost"

  val frontendScheme =
      environment.config.propertyOrNull("supertokens.frontend.scheme")?.getString()
          ?: throw IllegalStateException("supertokens.frontend.scheme not configured")
  val frontendHost =
      environment.config.propertyOrNull("supertokens.frontend.host")?.getString()
          ?: throw IllegalStateException("supertokens.frontend.host not configured")
  val frontendPath =
      environment.config.propertyOrNull("supertokens.frontend.path")?.getString()
          ?: throw IllegalStateException("supertokens.frontend.path not configured")

  val apiScheme =
      environment.config.propertyOrNull("supertokens.api.scheme")?.getString()
          ?: throw IllegalStateException("supertokens.api.scheme not configured")
  val apiHost =
      environment.config.propertyOrNull("supertokens.api.host")?.getString()
          ?: throw IllegalStateException("supertokens.api.host not configured")
  val apiPath =
      environment.config.propertyOrNull("supertokens.api.path")?.getString()
          ?: throw IllegalStateException("supertokens.api.path not configured")

  val smtpUser =
      environment.config.propertyOrNull("smtp.user")?.getString()
          ?: throw IllegalStateException("smtp.user not configured")
  val smtpPassword =
      environment.config.propertyOrNull("smtp.password")?.getString()
          ?: throw IllegalStateException("smtp.password not configured")
  val smtpHost =
      environment.config.propertyOrNull("smtp.host")?.getString()
          ?: throw IllegalStateException("smtp.host not configured")
  val smtpPort =
      (environment.config.propertyOrNull("smtp.port")?.getString()
              ?: throw IllegalStateException("smtp.port not configured"))
          .toInt()
  ...
  install(SuperTokens) {
    ...
    superTokens =
        superTokens(
          ...
          recipe(EmailPassword) { emailService = mailService }
          recipe(Sessions) {
          ...
          recipe(EmailVerification) { emailService = mailService }
          ...
          recipe(AccountLinking) {
            shouldDoAutomaticAccountLinking = { _, _ ->
              ShouldDoAccountLinkingResult(shouldAutomaticallyLink = true)
            }
          }
          recipe(Totp)
    ...
    jwtVerification { withAudience(frontendHost) }
  }
  ...
  install(StatusPages) { superTokens(catchGeneralError = true) }
