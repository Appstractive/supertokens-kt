  ...
  Scaffold(
      ...
      topBar = {
        TopAppBar(
            title = { Text("TOTP") },
            navigationIcon = {
              IconButton(onClick = { state.eventSink(TotpScreen.Event.GoBack) }) {
...
private fun AddDeviceView(
  ...
  var deviceName by rememberRetained { mutableStateOf(TextFieldValue()) }
  ...
  Button(onClick = { state.eventSink(TotpScreen.Event.CreateDevice(deviceName.text)) }) {
    Text("Create")
  }
}
...
private fun VerifyDeviceView(
  ...
  var totp by rememberRetained { mutableStateOf(TextFieldValue()) }
