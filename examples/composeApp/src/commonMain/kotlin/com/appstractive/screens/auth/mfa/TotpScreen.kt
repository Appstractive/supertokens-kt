...
fun TotpScreenPresenter(
    ...
    var pendingVerifyDeviceData: CreateTotpDeviceResult? by rememberRetained {
        mutableStateOf(null)
    }
    ...
    val eventHandler: (TotpScreen.Event) -> Unit = {
        when (it) {
            ...
            is TotpScreen.Event.CreateDevice -> apiCallController.call {
                pendingVerifyDeviceData = superTokensClient.getRecipe<TotpRecipe>().createDevice(
                    name = it.name,
                )
            }

            is TotpScreen.Event.Verify -> apiCallController.call {
                if(pendingVerifyDeviceData != null) {
                    superTokensClient.getRecipe<TotpRecipe>().verifyDevice(
                        name = pendingVerifyDeviceData!!.deviceName,
                        totp = it.totp,
                    )
                }
                else {
                    superTokensClient.getRecipe<TotpRecipe>().verify(
                        totp = it.totp,
                    )
                }
    ...
    return when {
        pendingVerifyDeviceData != null -> TotpScreen.State.VerifyDevice(
        ...
        mfaStatus == null -> TotpScreen.State.Loading(
            eventSink = eventHandler,
        )

        mfaStatus?.factors?.alreadySetup?.contains(AuthFactor.TOTP.key) == false -> TotpScreen.State.AddDevice(
            eventSink = eventHandler,
        )

        else -> TotpScreen.State.EnterTotp(
            eventSink = eventHandler,
        )
    }
}
