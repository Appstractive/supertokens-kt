...
fun PasswordlessScreenPresenter(
  ...
  var deviceId: String? by rememberRetained { mutableStateOf(null) }
  var preAuthSessionId: String? by rememberRetained { mutableStateOf(null) }

  val eventHandler: (PasswordlessScreen.Event) -> Unit = {
    when (it) {
      is PasswordlessScreen.Event.SignIn ->
          apiCallController.call {
            val result = superTokensClient.signUpWith(Passwordless) { email = it.email }
            deviceId = result.deviceId
            preAuthSessionId = result.preAuthSessionId
          }

      is PasswordlessScreen.Event.ConfirmCode ->
          apiCallController.call {
